...
$.fn.Huploadify = function(opts){
	var itemTemp = '<div class="photo uploading" id="${fileID}"><div class="photo-wrap"><div class="photo-img"><div class="progress"><div class="progress-bar"></div></div></div><div class="photo-info"><div class="photo-name">${fileName}</div><div class="photo-meta"><span class="icn-timer"></span>right now<span class="icn-storage"></span>${fileSize}</div></div></div></div>   ';
	var defaults = {
	...
	this.each(function(){
		...
		inputStr += '"/>';
		inputStr += '<a id="file_upload_'+instanceNumber+'-button" href="javascript:void(0)" class="btn btn-blue uploadify-button">';
	  ...
	  fileObj = {
		  ...
		  funSelect: function(files){
				for(var i=0,len=files.length;i<len;i++){
					...
					if(option.breakPoints){
				  	...
				  	$html.find('.progress-bar').css('width',initWidth);
					}
		  ...
		  onProgress: function(file, loaded, total) {
				var eleProgress = $('.photo-list').find('#fileupload_'+instanceNumber+'_'+file.index+' .progress');
				...
				var progressBar = eleProgress.children('.progress-bar');
